...
SUBROUTINE CALBAL1
  ...
  DO K=1,KC
    DO L=2,LA
      ...
      DYEBEG=DYEBEG+SCB(L)*DXYP(L)*HP(L)*DYE(L,K,1)*DZC(L,K)
...
SUBROUTINE CALBAL2
  ...
  DO K=1,KC
    DO LL=1,NCBS
      ...
      DYEOUT=DYEOUT-MIN(VHDX2(LN,K),0.)*DYE1(LN,K,1)-MAX(VHDX2(LN,K),0.)*DYE1(L,K,1)
  ...
  DO K=1,KC
    DO LL=1,NCBW
      ...
      DYEOUT=DYEOUT-MIN(UHDY2(LEC(L),K),0.)*DYE1(LEC(L),K,1)-MAX(UHDY2(LEC(L),K),0.)*DYE1(L,K,1)
  ...
  DO K=1,KC
    DO LL=1,NCBE
      ...
      DYEOUT=DYEOUT+MIN(UHDY2(L,K),0.)*DYE1(L,K,1)+MAX(UHDY2(L,K),0.)*DYE1(LWC(L),K,1)
  ...
  DO K=1,KC
    DO LL=1,NCBN
      ...
      DYEOUT=DYEOUT+MIN(VHDX2(L,K),0.)*DYE1(L,K,1)+MAX(VHDX2(L,K),0.)*DYE1(LS,K,1)
...
SUBROUTINE CALBAL3
  ...
  IF( ISTRAN(1) >= 1 )THEN
    ...
    DO NWR=1,NQWR
      ...
      SALOUT=SALOUT+( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
      IF( LD /= 1 .OR. LD /= LC )THEN
        SALOUT=SALOUT-( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,1))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,1)) )
      ENDIF
    ENDDO
  ENDIF
  IF( ISTRAN(3) >= 1 )THEN
    ...
    DO NS=1,NQSIJ
      ...
      NCSTMP=NCSERQ(NS,3)
      DO K=1,KC
        DYEOUT=DYEOUT-MAX(QSS(K,NS),0.)*CQS(K,NS,MD)-MIN(QSS(K,NS),0.)*DYE(L,K,MD)-MAX(QSERCELL(K,NS),0.)*CSERT(K,NCSTMP,3)-MIN(QSERCELL(K,NS),0.)*DYE(L,K,MD)
      ENDDO
    ENDDO
    DO NCTL=1,NQCTL
      RQWD=1.
    ...
    DO NWR=1,NQWR
      ...
      DYEOUT=DYEOUT+( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
      IF( LD /= 1 .OR. LD /= LC )THEN
        DYEOUT=DYEOUT-( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,3))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,3)) )
      ENDIF
    ENDDO
  ENDIF
  RETURN
  ...
  IF( NBAL == NTSMMT )THEN
    ...
    DO K=1,KC
      DO L=2,LA
        ...
        DYEEND=DYEEND+SCB(L)*DXYP(L)*HP(L)*DYE(L,K,1)*DZC(L,K)
