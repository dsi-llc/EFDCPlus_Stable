...
998     FORMAT(A11,8A9)
  ...
  DO ND=1,NDM
    ...
    IF( LDAYLIGHT .OR. IWQSUN /= 2 )THEN
      ...
      IF( ISTOPT(2) == 1 .OR. ISTOPT(2) >= 4 .OR. ISTRAN(8) > 0 )THEN
  ...
  IF( ISTOPT(2) == 1 )THEN
    ...
    IF( IASWRAD == 0. )THEN
      ...
      IF( LDAYLIGHT )THEN
        ...
        DO ND=1,NDM
          ...
          DO LP=LF,LL
            ...
            RADNET(L,KC) = RADNET(L,KC) + RSN*C1                                                  ! *** m*degC

            ! *** SAVE THE SOLAR RADIATION
            RADBOT(L,KC) = RADTOP(L,KC) - RSN                                                     ! *** W/m2
          ENDDO
        ENDDO   ! *** END OF DOMAIN
        !$OMP END DO
      ...
      ELSE
        ! *** NO SOLAR RADIATION.  FINALIZE THE TEMPERATURE FOR THE TOP LAYER
        K = KC
    ...
    DO ND=1,NDM
      ...
      DO LP=LF,LL
        ...
        IF( .NOT. ICECELL(L) )THEN
          ...
          ! *** DELME - For writing to EE_Arrays
          !PMCTESTX(1,L) = RHOWCP*HBLW   ! DELME  W/M2
          !PMCTESTX(2,L) = RHOWCP*HBCD   ! DELME  W/M2
          !PMCTESTX(3,L) = RHOWCP*HBCV   ! DELME  W/M2
          !PMCTESTX(4,L) = RHOWCP*HBEV   ! DELME  W/M2
          !PMCTESTX(5,L) = RHOWCP*( HBLW + HBCD + HBCV + HBEV )    ! DELME  W/M2
        ...
        DO LP=LF,LL
          ...
          IF( ABS(FLUXTB(L))/THICK > 0.001 )THEN
            ...
            IF( FLUXTB(L) < 0.0 )THEN
              TEM(L,KSZ(L)) = MAX(TEM(L,KSZ(L)),TEMB(L))    ! DELME - WHEN WAS THIS ADDED?
            ELSE
...
SUBROUTINE ICECOMP(ISTL_, DELTD2, ICESTEP)
  ...
  IF( ( NCTBC /= NTSTBC .OR. IS2TL > 0 ) .AND. (LCHECKICE .OR. LFRAZIL) )THEN
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ND, LF, LL, L, K, ICEITER)                     &
          ...
          IF( DELTD2 >= ICESTEP )THEN
            ...
            IF( ICEITER > ITERMAX2 )THEN
              ...
              IF( FMODE == 'WRITE    ' .OR. FMODE == 'READWRITE' )THEN
                WRITE(8, "(A,2I5,2F8.3,F10.4)" )'ICE TEMPERATURE: NEW MAX ITERATION (MAX L ICETHK ICET TIME): ',ICEITER,L,ICETHICK(L),ICETEMP(L),TIMEDAY

                IF( ICEITER > ITERMAX )THEN
                  WRITE(8,"(A,2I5,2F8.3,F10.4)")'ERROR: ICE TEMPERATURE ITERATIONS EXCEEDED MAX  ALLOWABLE: (MAX L ICETHK ICET TIME): ',ICEITER,L,ICETHICK(L),ICETEMP(L),TIMEDAY
                ENDIF
              ENDIF
              IF( ICEITER > ITERMAX )THEN
        ...
        ELSE
          ...
          IF( ISICE == 3 )THEN
            IF( ICETHICK(L) > 0.0 .AND. ICETHICK(L) < 1E-6 )THEN
              CALL EQUILIBRIUM_TEMPERATURE(WINDST(L), TDEWT(L), TATMT(L), SOLSWRT(L), ET, CSHE)
              IF( ET > 1.0 )THEN
                IF( DICETHW > 0. ) ICETHICK(L) = MAX(ICETHICK(L) - DICETHW,0.0)      ! *** PREVENT A FREEZE AND A THAW IN THE SAME TIME STEP FOR THE SAME CELL
                ! *** COMPLETE MELT
                IF( IS2TL == 0 )THEN
                  TMPVAL = ICEVOL(L)/DXYP(L)
...
SUBROUTINE SET_LIGHT(L)
  ...
  IF( ISTOPT(2) == 4 )THEN
    ...
    DO K = KC,KSZ(L),-1
      ...
      IF( K == KC )THEN
        ...
        IF( FRACLAYER > (1.0 - FSWRATF) .AND. HP(L) > 2.*HDRY ) FRACLAYER = 1.-FSWRATF
      ENDIF
  ...
  ELSEIF( ISTOPT(2) == 5 .OR. ISTRAN(8) > 0 )THEN
