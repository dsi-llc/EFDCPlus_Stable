...
SUBROUTINE SEDZLJ(L)
  ...
  REAL(RKD) ,DIMENSION(NSCM2) :: CTB
  REAL(RKD) ,DIMENSION(NSCM2) :: DEPBL
  REAL(RKD) ,DIMENSION(NSCM2) :: DEPTSS
  REAL(RKD) ,DIMENSION(NSCM2) :: DEPTSSB
  REAL(RKD) ,DIMENSION(NSCM)  :: ELAY
  REAL(RKD) ,DIMENSION(NSCM2) :: ETOT
  REAL(RKD) ,DIMENSION(NSCM2) :: PROB
  REAL(RKD) ,DIMENSION(NSCM)  :: PROBVR
  REAL(RKD) ,DIMENSION(NSCM2) :: QBFLUX
  REAL(RKD) ,DIMENSION(NSCM2) :: SMASS
  ...
  CSEDVR(1:NSCM)    = 0.0    ! *** Initialize van Rijn's equilibrium bedload concentration
  CTB(1:NSCM2)      = 0.0    ! *** Initialize bottom layer concentration 
  DEPBL(1:NSCM2)    = 0.0    ! *** Initialize deposition from bedload
  DEPTSS(1:NSCM2)   = 0.0    ! *** Initialize deposition from suspended load
  DEPTSSB(1:NSCM2)  = 0.0    ! *** Initialize deposition from suspended load collapsed into the bed class (propwash)
  ELAY(1:NSCM)      = 0.0    ! *** Initialize top-layer erosion rates for each sediment size class
  ETOT(1:NSCM2)     = 0.0    ! *** Initialize erosion rates for each size class and each cell
  PROB(1:NSCM2)     = 0.0    ! *** Initialize probability of deposition of suspended load
  PROBVR(1:NSCM)    = 0.0    ! *** 
  QBFLUX(1:NSCM2)   = 0.0    ! *** Initialize suspended load flux
  SMASS(1:NSCM2)    = 0.0    ! *** Initialize bedload sediment mass available for deposition  (g/cm^2)
  TTEMP(1:NSCM)     = 0.0    ! *** 
  ...
  IF( KSZ(L) == KC )THEN    ! *** Alberta
    ...
    ENDDO
  ELSE
    CTB(1:NSCM2) = SED(L,KSZ(L),1:NSCM2)*1.0D-06                        ! *** Convert bottom layer concentration from mg/L (g/m^3) to g/cm^3
  ENDIF
  ...
  IF( ICALC_BL > 0 .AND. PROP_ERO(L,0) == 0.0 )THEN
    ...
    ENDDO
    DEP = SUM(DEPBL(1:NSCM)) + SUM(DEPTSSB(1:NSCM))           ! *** Total deposition, sum of bedload and suspended load deposition for all size classes
  ELSE
    DEP = SUM(DEPTSSB(1:NSCM))                                ! *** Total deposition is the sum of suspended load deposition for all size classes
  ENDIF
  ...
  IF( NACTIVESHIPS > 0 )THEN                                                              ! *** Propwash is active and there are active ships. 
    PROP_ERO(L,1:NSCM) = PROP_ERO(L,1:NSCM)/DXYP(L)/10000.                                ! *** Convert mass from g to g/cm^2.  Ignore fast classes since not fractionated yet
  ENDIF
  ...
  ENDDO   ! ALL_LAYERS                                                               
  ETOTO(L) = SUM(ETOT(1:NSCM2))                                                      ! *** Total erosion in cell (g/cm^2)
  ERO = ETOTO(L)
  ...
  IF( DEP > 0.0 )THEN                                                                  
    ...
    IF( ICALC_BL > 0 )THEN
      PERSED(1:NSCM,1,L) = ( TTEMP(1:NSCM) + DEPTSSB(1:NSCM) + DEPBL(1:NSCM) )/TSED(1,L)  ! *** Bedload possible
    ELSE
      PERSED(1:NSCM,1,L) = ( TTEMP(1:NSCM) + DEPTSSB(1:NSCM) )/TSED(1,L)             ! *** No bedload
    ENDIF
  ENDIF
  ...
  IF( ICALC_BL > 0 )THEN
    ...
    ENDDO
  ELSE
    QBFLUX(1:NSCM2) = ETOT(1:NSCM2) - DEPTSS(1:NSCM2)       ! *** Net erosion (+) / Deposition (-) from TSS  (g/cm^2)
  ENDIF
  ...
  SEDF(L,0,1:NSCM2)     = QBFLUX(1:NSCM2)*10000._8/DTSEDJ                                               ! *** SEDF-Suspended Sediment flux (g/m^2/s), QBFLUX (g/cm^2)
  SED(L,KSZ(L),1:NSCM2) = SEDS(L,KSZ(L),1:NSCM2) + (SEDF(L,0,1:NSCM2)-SEDF(L,KSZ(L),1:NSCM2))*WDTDZ     ! *** SED-Suspended sediment concentration (g/m^3)
  ...
  IF( ICALC_BL > 0 )THEN
    ...
    ENDDO
    IF( NSCM2 > NSCM )THEN
      DO NS = NSCM+1, NSCM2                                                                       ! *** Fast class deposition volumes
