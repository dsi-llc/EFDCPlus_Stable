...
!
! !INTERFACE:
   subroutine diff_face(N,dt,cnpar,h,Bcup,Bcdw,Yup,Ydw,nuY,Lsour,Qsour,Y,ith)
!
! !DESCRIPTION:
!
! !USES:
...
!   set up upper boundary condition
   ...
   case(Neumann)
     if( N > 2 )then
       ...
       au(N-1,ith)  = -cnpar*a
       bu(N-1,ith) =  _ONE_ + cnpar*a - l
       du(N-1,ith) = (_ONE_ - (_ONE_-cnpar)*a)*Y(N-1)                  &
...
!   set up lower boundary condition
   ...
   case(Neumann)
     if( N > 2 )then   
       ...
       cu(1,ith)  = -cnpar*c
       bu(1,ith) =  _ONE_ + cnpar*c - l
       du(1,ith) = (_ONE_ - (_ONE_-cnpar)*c)*Y(1)                      &
...
!  solve linear system
   call tridiagonal(ith,N,1,N-1,Y)
