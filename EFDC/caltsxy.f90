...
SUBROUTINE CALTSXY  
  ...
  INTEGER :: ND, LF, LL, LP, L, LS, LN, I, J, NA, NW, M1, M2, NI, ICECOVL, IWMP, ITMP, IMAP, IICE, IERR
  ...
  IF(  .NOT. ALLOCATED(CLOUDTT) )THEN
    ...
    end if !***End calculation on master
    
  ENDIF

  ! *** *******************************************************************!
  ! INITIALIZE WIND SHELTERED SURFACE GAS TRANSFER
  IF( N == -1 .AND. (NWSER > 0 .OR. WIND.IFLAG > 0) )THEN
    if( process_id == master_id )THEN
        IF( DEBUG )THEN
...
SUBROUTINE WINDSTRESS(L)
  ...
  IMPLICIT NONE
  !REAL,DIMENSION(:), INTENT(IN) :: WINDSXX, WINDSXY, WINDSYX, WINDSYY
