...
2069  FORMAT(I4,3X,A2,5X,2E14.4,5X,2I5)
  ...
  IF( NTOX > 0 )THEN
    ...
    if( process_id == master_id )THEN
      CALL SEEK('C43B')
      DO NT=1,NTOX
        READ(1,*,IOSTAT=ISO) NDUM, ITOXKIN(1,NT) ,ITOXKIN(2,NT), ITOXKIN(3,NT), ITOXKIN(4,NT), ITOXKIN(5,NT)  !, ITOXKIN(6,NT)
        
        WRITE(7,1002)NCARD
        WRITE(7,*) NDUM, ITOXKIN(1,NT) ,ITOXKIN(2,NT), ITOXKIN(3,NT), ITOXKIN(4,NT), ITOXKIN(5,NT)  !, ITOXKIN(6,NT)
        IF( ISO > 0 ) GOTO 100
      ENDDO
      ...
      DO NT=1,NTOX
        IF( NSED > 0 )THEN
          DO NS=1,NSED
            ...
            WRITE(7,1002)NCARD
            WRITE(7,*) NDUM1, NDUM2, ITXPARW(NS,NT), CONPARW(NS,NT), TOXPARW(1,NS,NT), TOXPARB(1,NS,NT)
            IF( ISO > 0 ) GOTO 100
          ENDDO
        ENDIF
        IF( NSND > 0 )THEN
    ...
    if( process_id == master_id )THEN
      ...
      DO NT=1,NTOX
        ...
        WRITE(7,1002)NCARD
        WRITE(7,*) NDUM1, NDUM2, ITXPARWC(1,NT), CONPARWC(1,NT), TOXPARWC(1,NT), TOXPARBC(1,NT)
        IF( ISO > 0 ) GOTO 100
        
        READ(1,*,IOSTAT=ISO) NDUM1, NDUM2, ITXPARWC(2,NT), CONPARWC(2,NT), TOXPARWC(2,NT), TOXPARBC(2,NT)
        
        WRITE(7,1002)NCARD
        WRITE(7,*) NDUM1, NDUM2, ITXPARWC(2,NT), CONPARWC(2,NT), TOXPARWC(2,NT), TOXPARBC(2,NT)
        IF( ISO > 0 ) GOTO 100
      ENDDO
    endif
    ...
    IF( ANY(TOXPARW < 0.0) )THEN
      ...
      DO NT = 1,NTOX
        if( process_id == master_id )THEN
          ...
          DO L=2,LA_Global
            READ(1,*,IOSTAT=ISO) LDUM, IDUM, JDUM, (R2D_Global(L,NS),NS=1,NSCM)    ! *** TOXPARW
    ...
    IF( ANY(TOXPARB < 0.0) )THEN
      ...
      DO NT = 1,NTOX
        if( process_id == master_id )then    
          READ(1,*) TEXT                                                           ! *** Toxic ID
          DO L=2,LA_Global
            READ(1,*,IOSTAT=ISO) LDUM, IDUM, JDUM, (R2D_Global(L,NS),NS=1,NSCM)    ! *** TOXPARB
