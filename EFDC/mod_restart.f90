...
6667 FORMAT(7I5,2X,E12.4,2X,E12.4)
  ...
  DO L=2,LA
    ...
    H1V(L) = 0.5*(DXP(L)*DYP(L)*H1P(L)+DXP(LSC(L))*DYP(LWC(L))*H1P(LSC(L)))/(DXV(L)*DYV(L))
    P1(L)  = G*(H1P(L)+BELV(L))
    HU(L)  = 0.5*(DXP(L)*DYP(L)*HP(L)+DXP(LWC(L))*DYP(LWC(L))*HP(LWC(L)))/(DXU(L)*DYU(L))
    HV(L)  = 0.5*(DXP(L)*DYP(L)*HP(L)+DXP(LSC(L))*DYP(LWC(L))*HP(LSC(L)))/(DXV(L)*DYV(L))
    P(L)   = G*(HP(L)+BELV(L))
    HPI(L) = 1./HP(L)
    HUI(L) = 1./HU(L)
    HVI(L) = 1./HV(L)
...
SUBROUTINE Setup_Continuation_Files(RESTARTF)
  ...
  IF( ISTRAN(8) >= 1 )THEN
    ! ** wq_3dwc.jnp
      ...
      STR ='copy '//trim(RSTFILE4)//' wq_wcrst.inp'
      RESLOG = SYSTEMQQ(trim(str))
