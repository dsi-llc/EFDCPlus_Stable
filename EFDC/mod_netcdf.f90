...
! ----------------------------------------------------------------------
    ...
    integer(4), parameter        :: TWO=2, THREE=3, FOUR=4, TIMECNT=NF90_UNLIMITED, NC_VAR_CNT = 155, LEN_NAME = 20
    ...
    ncvar(1:NC_VAR_CNT) = (/&
        ...
        nc_var( 35, 'bed_cohesive',         'kg/m2',        0, 6, nc.KB_DIM, nc.SED_DIM,  'Bed cohesive sediment'), &
        nc_var( 36, 'bed_noncohesive',      'kg/m2',        0, 6, nc.KB_DIM, nc.SND_DIM,  'Bed non-cohesive sediment'), &
        ...
        nc_var(155, 'air_presssure',        'hPa',          0, 3, 0,         0,           'Barometric pressure') /)
    end function
    ...
    status = check_err(nf90_def_var(nc.id,'cell',NF90_UINT,(/ nc.cell_dim /),nc.nc_msh(7)),'def_cell_L')
    status = nf90_def_var_deflate(nc.id, nc.nc_msh(7), 1, 1, deflev)
    status = nf90_put_att(nc.id,nc.nc_msh(7),'long_name','cell index')

    status = check_err(nf90_def_var(nc.id,'col',NF90_UINT,(/ nc.cell_dim /),nc.nc_msh(8)),'def_cell_I')
    ...
    status = check_err(nf90_def_var(nc.id,'row',NF90_UINT,(/ nc.cell_dim /),nc.nc_msh(9)),'def_cell_J')
    ...
    if (IGRIDV > 0) then
        ...
        status = nf90_put_att(nc.id, nc.nc_kc, 'coordinates','latc lonc')
        ...
        status = nf90_put_att(nc.id, nc.nc_kl, 'coordinates','latc lonc')
        ...
        status = nf90_put_att(nc.id, nc.nc_sig, 'coordinates','latc lonc')
        ...
        status = nf90_put_att(nc.id, nc.nc_lyr, 'coordinates','latc lonc')
    else
