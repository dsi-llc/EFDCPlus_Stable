...
! ----------------------------------------------------------------------
    ...
    integer(4), parameter        :: TWO=2, THREE=3, FOUR=4, TIMECNT=NF90_UNLIMITED, NC_VAR_CNT = 156, LEN_NAME = 20
    ...
    type nc_dataset
        ...
        integer(4) :: nc_msh(6), nc_sig, nc_kl, nc_kc, nc_lyr
        integer(4) :: nc_lpt(4), nc_cuv, nc_rssbc, nc_seddia, lpt_dim
        integer(4) :: cell_dim, time_dim, name_dim
        integer(4) :: kc_dim, dye_dim, tox_dim, nalg_dim, nzoo_dim, nshf_dim
        integer(4) :: kb_dim, sed_dim, snd_dim, sxd_dim
    ...
    ncvar(1:NC_VAR_CNT) = (/&
        nc_var(  1, 'BELV',                 'm',            0, 3, 0,         0,           'Bottom elevation'), &
        nc_var(  2, 'HP',                   'm',            0, 3, 0,         0,           'Water depth'), &
        nc_var(  3, 'WSEL',                 'm',            0, 3, 0,         0,           'Water surface elevation'), &
        nc_var(  4, 'U',                    'm/s',          0, 4, nc.KC_DIM, 0,           'Eastward water velocity'), &
        nc_var(  5, 'V',                    'm/s',          0, 4, nc.KC_DIM, 0,           'Northward water velocity'), &
        nc_var(  6, 'W',                    'm/s',          0, 4, nc.KC_DIM, 0,           'Upward water velocity'), &
        nc_var(  7, 'DELT',                 'sec.',         0, 1, 0,         0,           'Time step'), &
        ...
        nc_var( 13, 'wind_x',               'm/s',          0, 3, 0,         0,           'Eastward wind velocity'), &
        nc_var( 14, 'wind_y',               'm/s',          0, 3, 0,         0,           'Northward wind velocity'), &
        nc_var( 15, 'wave_height',          'm',            0, 3, 0,         0,           'Significant wave height'), &
        nc_var( 16, 'wave_direction',       'degree',       0, 3, 0,         0,           'Wave direction'), &
        nc_var( 17, 'wave_period',          'sec.',         0, 3, 0,         0,           'Wave period'), &
        nc_var( 18, 'wave_dissipation',     'W/m2',         0, 3, 0,         0,           'Wave energy dissipation'), &
        nc_var( 19, 'wave_SXX',             'W/m2',         0, 3, 0,         0,           'Wave radian stress Sxx'), &
        nc_var( 20, 'wave_SXY',             'W/m2',         0, 3, 0,         0,           'Wave radian stress Sxy'), &
        nc_var( 21, 'wave_SYY',             'W/m2',         0, 3, 0,         0,           'Wave radian stress Syy'), &
        nc_var( 22, 'salinity',             'ppt',          0, 4, nc.KC_DIM, 0,           'Salinity'), &
        nc_var( 23, 'temperature',          'degC',         0, 4, nc.KC_DIM, 0,           'Water temperature'), &
        nc_var( 24, 'bed_temperature',      'degC',         0, 3, 0,         0,           'Bed temperature'), &
        nc_var( 25, 'evaporation',          'm/s',          0, 3, 0,         0,           'Evaporation'), &
        nc_var( 26, 'rainfall',             'm/s',          0, 3, 0,         0,           'Rainfall'), &
        ...
        nc_var( 30, 'bed_toxics',           'mg/kg',        0, 5, 0,         nc.TOX_DIM,  'Bed toxics'), &
        ...
        nc_var( 34, 'bed_porosity',         '-',            0, 3, 0,         0,           'Bed sediment porosity'), &
        nc_var( 35, 'bed_cohesive',         'kg/m2',        0, 5, 0,         nc.SED_DIM,  'Bed cohesive sediment'), &
        nc_var( 36, 'bed_noncohesive',      'kg/m2',        0, 5, 0,         nc.SND_DIM,  'Bed non-cohesive sediment'), &
        ...
        nc_var( 39, 'active_layer',         '-',            1, 4, nc.KB_DIM, 0,           'Active layer'), &
        ...
        nc_var( 43, 'bed_mass_fraction',    'percentage',   0, 6, nc.KB_DIM, nc.SXD_DIM,  'Sediment mass fraction'), &
        nc_var( 44, 'bed_d50',              'microns',      0, 3, 0,         0,           'Average particle size of bed surface'), &
        nc_var( 45, 'bed_erosion_rate',     'g/cm2/s',      0, 3, 0,         0,           'Total erosion rate in the cell'), &
        nc_var( 46, 'bed_deposition_rate',  'g/cm2/s',      0, 3, 0,         0,           'Total deposition rate in the cell'), &
        ...
        nc_var( 55, 'ice_thick',            'm',            0, 3, 0,         0,           'Ice thickness'), &
        ...
        nc_var( 76, 'ALG',                  'mg/L',         0, 6, nc.KC_DIM, nc.NALG_DIM, 'Algae'), &
        nc_var( 77, 'ZOO',                  'mg/L',         0, 6, nc.KC_DIM, nc.NZOO_DIM, 'Diatom algae'), &
        ...
        nc_var(117, 'WQRPS',                'gC/m2',        0, 3, 0,         0,           'Shoot carbon'), &
        nc_var(118, 'WQRPR',                'gC/m2',        0, 3, 0,         0,           'Root carbon'), &
        nc_var(119, 'WQRPE',                'gC/m2',        0, 3, 0,         0,           'Epiphyte carbon'), &
        nc_var(120, 'WQRPD',                'gC/m2',        0, 3, 0,         0,           'Detritus carbon'), &
        ...
        nc_var(123, 'HS_STATE',             '-',            1, 7, 0,         nc.NQCTL_DIM,'Hydraulic structure operation state'), &
        nc_var(124, 'HS_UNITS',             '-',            1, 7, 0,         nc.NQCTL_DIM,'Hydraulic structure operation units'), &
        nc_var(125, 'HS_CURVE',             '-',            1, 7, 0,         nc.NQCTL_DIM,'Hydraulic structure operation rating curve'), &
        ...
        nc_var(130, 'WR_STATE',             '-',            1, 7, 0,         nc.NQWR_DIM, 'Withdrawal/return operation state'), &
        ...
        nc_var(155, 'MOC',                  'kg',           0, 5, 0,         nc.LPT_DIM,  'Oil Mass'), &
        nc_var(156, 'air_presssure',        'hPa',          0, 3, 0,         0,           'Barometric pressure') /)
    end function
    ...
    integer :: status,colcnt,rowcnt,lyrcnt,nsxd,i,j,k,k1,l,c,r,m,empty(0)
    ...
    ! ** DEFINE DIMENSIONS
    status = check_err(nf90_def_dim(nc.id, 'TIME', timecnt, nc.time_dim),'time_dim');
    status = check_err(nf90_def_dim(nc.id, 'NODE', nc.nodecnt, nc.node_dim),'node_dim');
    status = check_err(nf90_def_dim(nc.id, 'CELL', nc.cellcnt, nc.cell_dim),'cell_dim');
    ...
    status = check_err(nf90_def_dim(nc.id, 'NPD', NPD, nc.lpt_dim),'lpt_dim');

    ! ** DEFINE VARIABLES
    status = check_err(nf90_def_var(nc.id,'mesh',NF90_UINT,empty, nc.nc_msh(1)),'def_mesh')
    status = nf90_put_att(nc.id,nc.nc_msh(1),'standard_name','')
    status = nf90_put_att(nc.id,nc.nc_msh(1),'cf_role','mesh_topology')
    status = nf90_put_att(nc.id,nc.nc_msh(1),'topology_dimension',(/ 2 /))
    status = nf90_put_att(nc.id,nc.nc_msh(1),'node_coordinates','lon lat')
    status = nf90_put_att(nc.id,nc.nc_msh(1),'face_coordinates','lonc latc')
    status = nf90_put_att(nc.id,nc.nc_msh(1),'face_node_connectivity','nv')
    !status = nf90_put_att(nc.id,nc.nc_msh(1),'edge_node_connectivity','ne')

    status = check_err(nf90_def_var(nc.id,'nv',NF90_UINT,(/ nc.cnr_dim, nc.cell_dim /),nc.nc_msh(2)),'def_nv')
    ...
    if (IGRIDV > 0) then
        ...
        status = check_err(nf90_def_var(nc.id,'sigma',NF90_FLOAT,(/ nc.kc_dim, nc.cell_dim/), nc.nc_sig),'def_sigma')
        ...
        status = nf90_put_att(nc.id,nc.nc_sig,'formula_terms','sigma: sigma eta: WSEL depth: Bottom')
        ...
        status = nf90_put_att(nc.id,nc.nc_lyr,'units','fraction')
    else
        status = check_err(nf90_def_var(nc.id,'sigma',NF90_FLOAT,(/ nc.kc_dim /), nc.nc_sig),'def_sigma')
        status = nf90_put_att(nc.id,nc.nc_sig,'standard_name','ocean_sigma_coordinate')
        status = nf90_put_att(nc.id,nc.nc_sig,'long_name','sigma at layer midpoints')
        status = nf90_put_att(nc.id,nc.nc_sig,'units','sigma_level')
        status = nf90_put_att(nc.id,nc.nc_sig,'positive','up')
    ...
    ! *** LPT
    if( ISNCDF(9) > 0 ) then ! .and. JSEXPLORER == -1
        status = check_err(nf90_def_var(nc.id,'trk_lat',nf90_float,(/nc.lpt_dim,nc.time_dim/),nc.nc_lpt(2)),'def_trk_lat')
        status = nf90_def_var_deflate(nc.id, nc.nc_lpt(2), 1, 1, deflev)
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'standard_name','')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'long_name','drifter latitude')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'units','degrees_north')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'valid_min','-90')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'valid_max','90')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'grid_mapping','crs')
        status = nf90_put_att(nc.id,nc.nc_lpt(2),'_FillValue',missing_value)

        status = check_err(nf90_def_var(nc.id,'trk_lon',nf90_float,(/nc.lpt_dim,nc.time_dim/),nc.nc_lpt(1)),'def_trk_lon')
        status = nf90_def_var_deflate(nc.id, nc.nc_lpt(1), 1, 1, deflev)
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'standard_name','')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'long_name','drifter longitude')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'units','degrees_east')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'valid_min','-180')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'valid_max','180')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'grid_mapping','crs')
        status = nf90_put_att(nc.id,nc.nc_lpt(1),'_FillValue',missing_value)

        status = check_err(nf90_def_var(nc.id,'trk_lev',nf90_float,(/nc.lpt_dim,nc.time_dim/), nc.nc_lpt(3)),'def_trk_sigma')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'standard_name','')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'long_name','drifter elevation')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'units','m')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'grid_mapping','crs')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'coordinates','TRK_LAT TRK_LON')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'_CoordinateZisPositive','up')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'_CoordinateTransformType','Vertical')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'_CoordinateAxisType','GeoZ')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'_CoordinateAxes','lev')
        status = nf90_put_att(nc.id,nc.nc_lpt(3),'_FillValue',MISSING_VALUE)

        if( any(isoilspi == 1) )then
            call def_var(nc, ncvar, 155)

            status = check_err(nf90_def_var(nc.id,'trk_vol',nf90_float,(/nc.lpt_dim,nc.time_dim/), nc.nc_lpt(4)),'def_trk_vol')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'standard_name','')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'long_name','drifter oil volume')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'units','m3')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'grid_mapping','crs')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'coordinates','TRK_LAT TRK_LON')
            status = nf90_put_att(nc.id,nc.nc_lpt(4),'_FillValue',MISSING_VALUE)
        endif
    endif
    ! *** END LPT
    ...
    status = check_err(nf90_put_att(nc.id,nf90_global,'Conventions','CF-1.4'),'att_cf')
    ...
    status = check_err(nf90_put_var(nc.id, nc.nc_msh(3), lon),'lon')
    status = check_err(nf90_put_var(nc.id, nc.nc_msh(4), lat),'lat')
    status = check_err(nf90_put_var(nc.id, nc.nc_msh(5), lonc),'lonc')
    status = check_err(nf90_put_var(nc.id, nc.nc_msh(6), latc),'latc')

    deallocate(lon, lat, lonc, latc, nv)
    ...
    if ( IGRIDV > 0 )then ! Sigma-Zed level
        ...
        do L = 2,LA_Global ! *** Modified to handle global
            ...
            J = sorted_loc_to_glob(L).global_j - JMN_Global + 1
            k1 = KSZ_Global(L)
            kcsgz(L-1) = KC - KSZ_Global(L) + 1    ! Number of vertical layers
            sumdzc = 0.0
            do k=1,KC
    ...
    status = check_err(nf90_put_var(nc.id, nc.nc_msh(4), lon),'lon')
    ...
    if ( ISSPH(8) >= 1 ) then
        ...
        if( ISNCDF(1) > 0 ) call def_var(nc, ncvar, 22) ! *** Salinity

        if( ISNCDF(2) > 0 )then
            call def_var(nc, ncvar, 23) ! *** Temperature
            if( TEMBO > 0.) call def_var(nc, ncvar, 24) ! *** Bed temperature
            if( IEVAP > 1 )then
                call def_var(nc, ncvar, 25) ! *** Evaporation
                call def_var(nc, ncvar, 26) ! *** Rainfall
            endif
        endif

        if( ISNCDF(3) > 0 ) call def_var(nc, ncvar, 27) ! *** Dye

        if( ISNCDF(4) > 0 ) call def_var(nc, ncvar, 28) ! *** Shellfish larvae

        if( ISNCDF(5) > 0 )then
            call def_var(nc, ncvar, 29) ! *** Toxics
            call def_var(nc, ncvar, 30) ! *** Bed toxics
        ENDIF

        if( ISNCDF(6) > 0 .OR. ISNCDF(7) > 0 )then
            do i=32,34
                call def_var(nc, ncvar, i)
            enddo
            if( ISNCDF(6) > 0 ) then
                call def_var(nc, ncvar, 37)     ! *** Cohesive sediment concentration
                call def_var(nc, ncvar, 35)     ! *** Bed cohesive sediment
            endif
            if( ISNCDF(7) > 0 )then
                call def_var(nc, ncvar, 38)     ! *** Noncohesive sediment concentration
                call def_var(nc, ncvar, 36)     ! *** Bed noncohesive sediment
        ...
        if( ISNCDF(2) > 0 .AND. ISICE  >= 3 )then
    ...
    if( LSEDZLJ )then
        ...
        if( ISNCDF(5) > 0 )then
            call def_var(nc, ncvar, 30) ! *** Bed toxics
            if( ICALC_BL > 0 )then
                call def_var(nc, ncvar, 50) ! *** Bedload toxic concentration (mg/m^2)
            endif
        endif
        ...
        if( ISBEXP >= 1 .and. KB > 1  .AND. ISSPH(8) < 1 ) then
            if( ISNCDF(6) > 0 .OR. ISNCDF(7) > 0 ) then
                ...
                if( ISNCDF(6) > 0 )then
                    call def_var(nc, ncvar, 35) ! *** Bed cohesive
                endif
                if( ISNCDF(7) > 0 )then
                    call def_var(nc, ncvar, 36) ! *** Bed noncohesive
                endif
                if( ISNCDF(5) > 0 )then
                    call def_var(nc, ncvar, 30) ! *** Bed toxics
                endif
    ...
    if( ISNCDF(8) > 0 )then
        do i=57,74
            call def_var(nc, ncvar, i)
        enddo
        if(NALGAE > 0) call def_var(nc, ncvar, 76)     ! *** Algal groups
        if(NZOOPL > 0) call def_var(nc, ncvar, 77)     ! *** Zooplankton groups
    ...
    if (NASER > 0 .OR. ICYCLONE > 0) then
        call def_var(nc, ncvar, 156)
    endif
    ...
    !if( ncvar(i).id > 0 )then
        ...
        case (2)
            status = nf90_put_att(nc.id, id, 'coordinates','latc lonc')
        case (3)
            status = nf90_put_att(nc.id, id, 'coordinates','time latc lonc')
        case (4)
            status = nf90_put_att(nc.id, id, 'coordinates','time sigma latc lonc')
        case (5)
            !status = nf90_put_att(nc.id, id, 'coordinates','time class latc lonc')
            status = nf90_put_att(nc.id, id, 'coordinates','time latc lonc')
        case (6)
            !status = nf90_put_att(nc.id, id, 'coordinates','time class sigma latc lonc')
            status = nf90_put_att(nc.id, id, 'coordinates','time sigma latc lonc')
        case (7)
        ...
        case (8)
            ...
            status = nf90_put_att(nc.id, id, 'coordinates','time sigma')
        end select        
        if (ncvar(i).dim_type >= 2 .and. ncvar(i).dim_type <= 6) then
            status = nf90_put_att(nc.id, id, 'mesh', 'mesh')
            status = nf90_put_att(nc.id, id, 'location', 'cell')
        endif
...
998 close(ucor)
    ...
    character(*),intent(in) :: msg
    real(4), allocatable, dimension(:,:) :: values
    integer :: i,j,L,k,k1,m,ip, count,status,str3d(3),cnt3d(3)

    str3d = (/ 1, 1, nc.time_idx/)
    if (is > 0) then
        cnt3d = (/ NPNT(is), ncmp, 1/)
        allocate(values(NPNT(is), ncmp))
        do ip = 1, NPNT(is)
            i = HFREGRP(is).ICEL(ip)
            j = HFREGRP(is).JCEL(ip)
            L = LIJ_Global(i,j)
            k = KBT_Global(L)
            do m = 1,ncmp
                values(ip, m) = factor * array3d(L, k, m)
            enddo
        enddo
    else
        count = LA_Global-1
        ...
        do i=1, count
            ...
            k = KBT_Global(L)
            do m = 1,ncmp
                values(i, m) = factor * array3d(L, k, m)
            enddo
        enddo
    endif    
    ...
    if( ISTRAN(6) > 0 .OR. ISTRAN(7) > 0 .OR. BATHY.IFLAG > 0 .OR. nc.time_idx <= 1)then
    ...
    if ( ISSPH(8) >= 1 ) then
        ...
        if( ISNCDF(1) > 0 ) then
            ! *** Salinity
            status = nc_write_wc_2d(nc, is, 22, SAL_Global, KC, 1., 'Salt')
        endif

        if( ISNCDF(2) > 0 )then
            ! *** Temperature
            status = nc_write_wc_2d(nc, is, 23, TEM_Global, KC, 1., 'TEM')
            if( TEMBO > 0.) then
                ! *** Bed temperature
                status = nc_write_cell_1d(nc, is, 24, TEMB_Global, 1., 'TEMB')
            endif
            if( IEVAP > 1 )then
                ! *** Evaporation
                status = nc_write_cell_1d(nc, is, 25, EVAPT_Global, 1., 'EVAPT')
                ! *** Rainfall
                status = nc_write_cell_1d(nc, is, 26, RAINT_Global, 1., 'RAINT')
            endif
        endif

        if( ISNCDF(3) > 0 ) then
            ! *** Dye
            status = nc_write_wc_3d(nc, is, 27, DYE_Global, KC, NDYE, 1.0, 'DYE')
        endif

        if( ISNCDF(4) > 0 ) then
            ! *** Shellfish larvae
            status = nc_write_wc_2d(nc, is, 28, SFL_Global, KC, 1., 'SFL')
        endif

        if( ISNCDF(5) > 0 ) then
            ! *** Toxics
            status = nc_write_wc_3d(nc, is, 29, TOX_Global, KC, NTOX, 1.0, 'TOX')
            ! *** Bed toxics
            status = nc_write_bed_3d(nc, is, 30, TOXB_Global, NTOX, 1., 'TOXB')
        ENDIF
        ...
        if( ISNCDF(6) > 0 .OR. ISNCDF(7) > 0 )then
            ...
            if( ISNCDF(6) > 0 ) then
                ...
                status = nc_write_wc_3d(nc, is, 37, SED_Global, KC, NSED, 1.0, 'SED')
                ! *** Bed cohesive sediment
                status = nc_write_bed_3d(nc, is, 35, SEDB_Global, NSED, 1., 'SEDB')
            endif
            if( ISNCDF(7) > 0 )then
                ! *** Noncohesive sediment concentration
                status = nc_write_wc_3d(nc, is, 38, SND_GLobal, KC, NSND, 1.0, 'SND')
                ! *** Bed noncohesive sediment
                status = nc_write_bed_3d(nc, is, 36, SNDB_Global, NSND, 1., 'SNDB')
                !WRITE(EE_UNIT) ((REAL(SNDB_Global(L,KBT_Global(L),NX),4), L=2,LA_Global), NX=1,NSND)
                !WRITE(EE_UNIT) (((REAL(SND_GLobal(L,K,NX),4), K=KSZ_Global(L),KC), L=2,LA_Global), NX=1,NSND)
                if( ICALC_BL > 0 .AND. NSND > 0 )then
                    ! *** CALCULATE EQUIVALENT CONCENTRATIONS
                    ! *** Bedload_x
                    status = nf90_put_var(nc.id, nc.idx(48), real(QSBDLDX_Global(2:LA_Global,:),4), (/1, 1, nc.time_idx/), (/NSND, nc.cellcnt, 1/))
                    ! *** Bedload_y
                    status = nf90_put_var(nc.id, nc.idx(49), real(QSBDLDY_Global(2:LA_Global,:),4), (/1, 1, nc.time_idx/), (/NSND, nc.cellcnt, 1/))
                endif
            endif
        endif
        if( ISGWIE > 0 )then
    ...
    if( LSEDZLJ )then
        ...
        deallocate(array2d)
        ! *** PERSED(NSCM,KB,LCM) - This is the mass percentage of each size class in a layer
        !status = nf90_put_var(nc.id, nc.idx(43), PERSED_Global(:,:,2:LA_Global), (/1, 1, 1, nc.time_idx/), (/NSCM, KB, LA_Global-1, 1/))
        allocate(array3d(LA_Global, KB, NSCM))
        do L=2,LA_Global
            do K=1,KB
                do i=1,NSCM
            ...
            enddo
        enddo
        status = nc_write_cell_3d(nc, is, 43, array3d, KB, NSCM, 1., 'PERSED')
        deallocate(array3d)
        ! *** D50AVG(LCM)   - Average particle size of bed surface (microns)
        status = nc_write_cell_1d(nc, is, 44, real(D50AVG_Global), 1., 'D50AVG')
        ! *** ETOTO(LCM)    - Total erosion rate in the cell g/cm^2/s
        status = nc_write_cell_1d(nc, is, 45, real(ETOTO_Global), 1., 'ETOTO')
        ! *** DEPO(LCM)     - Total deposition rate in the cell g/cm^2/s
        status = nc_write_cell_1d(nc, is, 46, real(DEPO_Global), 1., 'DEPO')

        if( ICALC_BL > 0 )then
            ! *** Bedload mass inb g/m^2 for each size class
            status = nc_write_cell_2d(nc, is, 47, real(CBL_Global), NSCM, 10000., 'CBL')
            ! *** Bedload flux in X direction (g/s)
            status = nc_write_cell_2d(nc, is, 48, real(QSBDLDX_Global), NSCM, 1., 'QSBDLDX')
            ! *** Bedload flux in Y direction (g/s)
            status = nc_write_cell_2d(nc, is, 49, real(QSBDLDY_Global), NSCM, 1., 'QSBDLDY')
        endif
        ...
        if( ISBEXP >= 1 .AND. KB > 1 .AND. ISSPH(8) < 1) then
            if( ISNCDF(6) > 0 .OR. ISNCDF(7) > 0 ) then
                ...
                if( ISNCDF(6) > 0 ) then
                    ! *** Bed cohesive
                    status = nc_write_cell_3d(nc, is, 35, SEDB_Global, KB, NSED, 1., 'SEDB')
                endif
                if( ISNCDF(7) > 0 ) then
                    ! *** Bed noncohesive
                    status = nc_write_cell_3d(nc, is, 36, SNDB_Global, KB, NSND, 1., 'SNDB')
                endif
                if( ISNCDF(5) > 0 )then
                    ! *** Bed toxics
    ...
    if( ISNCDF(8) > 0 )then
        do i=1,18
            if( ISTRWQ(i) > 0 )then
                status = nc_write_wc_2d(nc, is, 56+i, WQV_Global(:,:,i), KC, 1., 'WQ')
            endif
        enddo
        ! *** Algal groups
        if(NALGAE > 0) then
            k1 = 20
            k2 = 19 + NALGAE
            status = nc_write_wc_3d(nc, is, 76, WQV_Global(:,:,k1:k2), KC, NALGAE, 1., 'ALGAE')
        endif
        if(NZOOPL > 0) then     ! *** Zooplankton groups
            k1 = 19 + NALGAE + 1
            k2 = k1 + NZOOPL - 1
            status = nc_write_wc_3d(nc, is, 77, WQV_Global(:,:,k1:k2), KC, NZOOPL, 1., 'ZOOP')
        endif
        if( IWQBEN > 0 .and. ISSDBIN /= 0 ) then
    ...
    if (NASER > 0 .OR. ICYCLONE > 0) then
        status = nc_write_cell_1d(nc, is, 156, PATMT_Global, 1., 'Pa')
    endif
    ...
    if( ISNCDF(9) > 0 ) then ! .and. JSEXPLORER == -1
        ...
        do i=1,NPD
            ...
            array2d(2,i) = yll(1)
        enddo
        status = nc_write_array1d(nc, nc.nc_lpt(1), array2d(1,:), 0, NPD, 1., 'XLA')
        status = nc_write_array1d(nc, nc.nc_lpt(2), array2d(2,:), 0, NPD, 1., 'YLA')
        status = nc_write_array1d(nc, nc.nc_lpt(3), real(ZLA), 0, NPD, 1., 'ZLA')
        if( any(isoilspi == 1) )then
            status = nc_write_array1d(nc, nc.nc_lpt(4), real(DVOL), 0, NPD, 1., 'DVOL')
            status = nc_write_cell_1d(nc, is, 155, MOC, 1., 'MOC')
        endif
        deallocate(array2d)
    endif
    ! *** END LPT
