...
34569 FORMAT(8I8)
  ...
  ! *** ***********************************************************************
  FORALL(NS=1:NSCM2) SSGI(NS) = 1.0/(1.0E6*SSG(NS))  !initialize SSGI.  Specific Volume (m**3/g)
  ...
  WSEDO(1:NSCM) = DWS(1:NSCM)/100.0_8      ! *** WSEDO  - Fixed/Specified settling velocity.  Convert from cm/s (DWS) to m/s (WSEDO)
  SEDDIA(1:NSCM) = D50(1:NSCM)/1.d6        ! *** SEDDIA - Nominal diameter for each sediment class, even cohesives.  Microns to m.
