...
! ***   THEREFORE, THE MASS BALANCE FOR TOXICS WILL BE IN MG INSTEAD OF GRAMS FOR THE OTHER PARAMETERS
  ...
  IMPLICIT NONE                                                                                                          
  INTEGER :: NS,L,K,NT,LN,LE,MD                                                                                          

  IF( NBAL > 1 ) RETURN

  !OPEN( 555, FILE=OUTDIR//'OPENBC_FLUXES.DAT',STATUS='UNKNOWN')   ! *** DELME
  ...
  PPEBEG=0.
  BBEBEG=0.
  DYEBEG2T=0.0      ! delme -  duplicate of DYEBEG
  DO NS=1,NSED
    SEDBEG2T(NS)=0.0
    SEDBEG2TW(NS)=0.0
    SEDBEG2TB(NS)=0.0
  ...
  DO MD=1,NDYE
    DO K=1,KC
      DO L=2,LA
        DYEBEG   = DYEBEG   + SCB(L)*DXYP(L)*HP(L)*DYE(L,K,MD)*DZC(L,K)
        DYEBEG2T = DYEBEG2T + SCB(L)*DXYP(L)*HP(L)*DYE(L,K,MD)*DZC(L,K)
      ENDDO
    ENDDO
  ENDDO
...
SUBROUTINE BAL2T2
  ...
  DO K=1,KC
    DO LL=1,NCBS
      ...
      DO MD=1,NDYE
        DYEOUT  = DYEOUT  - DELT*MIN(VHDX2(LN,K),0.)*DYE(LN,K,MD) - DELT*MAX(VHDX2(LN,K),0.)*DYE(L,K,MD)
      ENDDO
      DO NS=1,NSED
  ...
  DO K=1,KC
    DO LL=1,NCBW
      ...
      DO MD=1,NDYE
        DYEOUT  = DYEOUT  - DELT*MIN(UHDY2(LEC(L),K),0.)*DYE(LEC(L),K,MD) - DELT*MAX(UHDY2(LEC(L),K),0.)*DYE(L,K,MD)
      ENDDO
      DO NS=1,NSED
  ...
  DO K=1,KC
    DO LL=1,NCBE
      ...
      DO MD=1,NDYE
        DYEOUT  = DYEOUT  + DELT*MIN(UHDY2(L,K),0.)*DYE(L,K,MD)+DELT*MAX(UHDY2(L,K),0.)*DYE(LWC(L),K,MD)
      ENDDO
      DO NS=1,NSED
  ...
  DO K=1,KC
    DO LL=1,NCBN
      ...
      DO MD=1,NDYE
        DYEOUT = DYEOUT   + DELT*MIN(VHDX2(L,K),0.)*DYE(L,K,MD)+DELT*MAX(VHDX2(L,K),0.)*DYE(LS,K,MD)
      ENDDO
      DO NS=1,NSED
...
SUBROUTINE BAL2T3A
  ...
  IF( ISTRAN(1) >= 1 )THEN
    ...
    DO NWR=1,NQWR
      ...
      SALOUT = SALOUT + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
      IF( LD /= 1 .OR. LD /= LC )THEN
        SALOUT=SALOUT - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,1))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,1)) )
      ENDIF
    ENDDO
  ENDIF
  ...
  IF( ISTRAN(3) >= 1 )THEN
    DO MD=1,NDYE
      ...
      DO NS=1,NQSIJ
        ...
        NCSTMP=NCSERQ(NS,3)
        DO K=1,KC
          DYEOUT   = DYEOUT   - DELT*MAX(QSS(K,NS),0.)*CQS(K,NS,M) - DELT*MIN(QSS(K,NS),0.)*DYE(L,K,MD) - DELT*MAX(QSERCELL(K,NS),0.)*CSERT(K,NCSTMP,M) - DELT*MIN(QSERCELL(K,NS),0.)*DYE(L,K,MD)
          DYEOUT2T = DYEOUT2T - DELT*MAX(QSS(K,NS),0.)*CQS(K,NS,M) - DELT*MIN(QSS(K,NS),0.)*DYE(L,K,MD) - DELT*MAX(QSERCELL(K,NS),0.)*CSERT(K,NCSTMP,M) - DELT*MIN(QSERCELL(K,NS),0.)*DYE(L,K,MD)  !DELME-DUPLICATE
        ENDDO
      ENDDO
      ...
      DO NWR=1,NQWR
        ...
        DYEOUT = DYEOUT   + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
        DYEOUT2T=DYEOUT2T + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
        IF( LD /= 1 .OR. LD /= LC )THEN
          DYEOUT   = DYEOUT   - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,M))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
          DYEOUT2T = DYEOUT2T - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,M))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
        ENDIF
      ENDDO
    ENDDO
  ENDIF
  ...
  IF( ISTRAN(5) >= 1 )THEN
    DO NT=1,NTOX
      ...
      DO NWR=1,NQWR
        ...
        TOXOUT2T(NT) = TOXOUT2T(NT) + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
        IF( LD /= 1 .OR. LD /= LC )THEN
          TOXOUT2T(NT) = TOXOUT2T(NT) - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,M))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
        ENDIF
      ENDDO
    ENDDO
  ENDIF
  ...
  IF( ISTRAN(6) >= 1 )THEN
    DO NSX=1,NSED
      ...
      DO NWR=1,NQWR
        ...
        SEDOUT2T(NSX) = SEDOUT2T(NSX) + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
        IF( LD /= 1 .OR. LD /= LC )THEN
          SEDOUT2T(NSX) = SEDOUT2T(NSX) - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,M))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
        ENDIF
      ENDDO
    ENDDO
  ENDIF
  ...
  IF( ISTRAN(7) >= 1 )THEN
    DO NSX=1,NSND
      ...
      DO NWR=1,NQWR
        ...
        SNDOUT2T(NSX) = SNDOUT2T(NSX) + DELT*( (WITH_RET(NWR).QWR+QWRABS)*CONT(LU,KU) )
        IF( LD /= 1 .OR. LD /= LC )THEN
          SNDOUT2T(NSX) = SNDOUT2T(NSX) - DELT*( WITH_RET(NWR).QWR*(CONT(LU,KU)+CQWR(NWR,M))+QWRABS*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
        ENDIF
      ENDDO
    ENDDO
  ENDIF
    ...
    DO NT=1,NTOX
      ...
      IF( IBALSTDT == 1 )THEN
        ...
        DO L=2,LA
          ...
          TOXFLUXB2T(NT)=TOXFLUXB2T(NT)+DTSED*DXYP(L)*TOXFB(L,NT)
...
SUBROUTINE BAL2T5
  ...
  DYEEND2T=0.0   ! delme - duplicate of DYEEND
  DO NS=1,NSED
    SEDEND2T(NS)=0.0
    SEDEND2TW(NS)=0.0
    SEDEND2TB(NS)=0.0
  ...
  DO K=1,KC
    DO L=2,LA
      ...
      DO MD=1,NDYE  !2018-10-25, NTL: Fixed bug unassigned MD
        DYEEND   = DYEEND   + SCB(L)*DXYP(L)*HP(L)*DYE(L,K,MD)*DZC(L,K)
        DYEEND2T = DYEEND2T + SCB(L)*DXYP(L)*HP(L)*DYE(L,K,MD)*DZC(L,K)
      ENDDO
  ...
  if( process_id == master_id )THEN
      ...
      NS=0
      WRITE(89,950)NS,DYEBEG2T,DYEOUT2T,DYEBMO2T,DYEEND2T,DYEERR2T,RDERDE2T,RDERDO2T   ! DELME - REMOVE THIS AFTER FINAL TESTING
