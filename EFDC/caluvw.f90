          ...
          IF( HPPTMP <= 0. .AND. HP(L) > 0. )THEN
            ...
            IF( HP(L) <= HDRY .AND. ISDRY > 0 )THEN
              ...
              if( process_id == master_id )THEN
                ...
                WRITE(8,'(A,I10,F12.4,I5,3F10.4)')'           NITER,TIMEDAY,L,H2P,HP,HPNEW',NITER,TIMEDAY,map2global(L).LG,H2P(L),HP(L),HPPTMP
              end if
          ...
          IF( HPPTMP <= 0. .AND. HP(L) > 0. )THEN
            ...
            IF( HP(L) <= HDRY .AND. ISDRY > 0 )THEN
              ...
              if( process_id == master_id )THEN
                ...
                ENDIF
                WRITE(8,'(A,I5,3F10.4,F12.4)')    ' WARNING!  NEG DEPTH IN CONTINUITY CHECK:'
                WRITE(8,'(A,I10,F12.4,I5,3F10.4)')'                N,TIMEDAY,L,H1P,HP,HPNEW',N,TIMEDAY,map2global(L).LG,H1P(L),HP(L),HPPTMP
