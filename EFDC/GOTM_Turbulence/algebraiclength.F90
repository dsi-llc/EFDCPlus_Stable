...
!
! !INTERFACE:
   subroutine algebraiclength(method,nlev,z0b,z0s,depth,h,NN,ith)
!
! !DESCRIPTION:
   ...
   select case (method)
      ...
      case(parabolic)
         ...
         do i = 1,nlev-1
            db = db+h(i)
            ds = depth-db
            Lgs(i,ith) = kappa*(ds+z0s)*(db+z0b)/(ds+db+z0b+z0s)
         enddo
...
! triangular shape
      ...
      case(triangular)
         do i = 1,nlev-1
            db = db+h(i)
            ds = depth-db
            Lgs(i,ith) = kappa*min(ds+z0s,db+z0b)
         enddo
...
! modification of parabolic mixing length
      case(Xing_Davies)
         ...
         do i = 1,nlev-1
           ...
           Lgs(i,ith) = kappa*(ds+z0s)*(dbxing+z0b)/(ds+dbxing+z0s+z0b)
         enddo
...
! modification of parabolic mixing length
      case(Robert_Ouellet)
         do i = 1,nlev-1
           ...
           ds = depth-db
           ! lu changed a bug in here
           Lgs(i,ith)= kappa*(db+z0b)*sqrt(1-(db-z0s)/depth)
         enddo

         Lgs(0   ,ith) = kappa*z0b
         Lgs(nlev,ith) = kappa*(depth+z0b)*sqrt(z0s/depth) ! no log-law (!)
!
! Blackadar (1962).
! In the form suggested by Luyten et al. (1996) for two boundary layers.
      case(Blackadar)
         ...
         do i = 1,nlev-1
            ...
            int_qz = int_qz + sqrt(tke(i,ith))*(db+z0b)*h(i)
            ! compute vertically averaged turbulent velocity
            int_q  = int_q  + sqrt(tke(i,ith))*h(i)
         enddo
         ...
         do i = 1,nlev-1
            db = db+h(i)
            ds = depth-db
             Lgs(i,ith) = 1/(1/(kappa*(ds+z0s))+1/(kappa*(db+z0b))+1/La)
         enddo
...
!  Ispramix
      case(ispra_length)
         call ispralength(nlev,NN,h,depth,ith)
