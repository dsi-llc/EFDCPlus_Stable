...
!
! !INTERFACE:
   subroutine tkeeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
...
!
   ...
   tkeo(:,ith) = tke(:,ith)
   avh(0)    = num(0   ,ith)/sig_k
   avh(nlev) = num(nlev,ith)/sig_k
   do i = 1,nlev-1

!     compute diffusivity
      avh(i) = num(i,ith)/sig_k
...
!  (only for output)
   tke(nlev,ith)  = k_bc(Dirichlet,ubc_type,z0s,z0s,u_taus)
   tke(0   ,ith)  = k_bc(Dirichlet,lbc_type,z0b,z0b,u_taub)
