...
!BOC
     ...
     do i=1,nlev-1
        ...
        Pe   =   ( P(i,ith) + B(i,ith) )/eps(i,ith)
        Pbeb =   Pb(i,ith)/epsb(i,ith)
        r_i  =   1./r(i)
        ...
        nCm  =  n0  +  n1*an(i,ith) +  n2*as(i,ith) + n3*at(i,ith)
        nCmp =  nt0 + nt1*an(i,ith) + nt2*as(i,ith)

        nGam = ( gam0 + gam1*an(i,ith) + gam2*as(i,ith) )*at(i,ith)
