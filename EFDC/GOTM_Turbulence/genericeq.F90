...
!
! !INTERFACE:
   subroutine genericeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
...
!  re-construct psi at "old" timestep
   do i = 0,nlev
      ...
      psi(i) = cm0**gen_p * tkeo(i,ith)**gen_m * Lgs(i,ith)**gen_n
   enddo
...
!     compute production terms in psi-equation
      PsiOverTke  = psi(i)/tkeo(i,ith)
      prod        = cpsi1*PsiOverTke*P(i,ith)
      buoyan      = cpsi3*PsiOverTke*B(i,ith)
      diss        = cpsi2*PsiOverTke*eps(i,ith)
...
!  do diffusion step
   call diff_face(nlev,dt,cnpar,h,psi_ubc,psi_lbc,                          &
                  DiffPsiup,DiffPsidw,avh,Lsour,Qsour,psi,ith)


!  fill top and bottom value with something nice
!  (only for output)
   psi(nlev)  = psi_bc(Dirichlet,ubc_type,z0s,tke(nlev,ith),z0s,u_taus)
   psi(0   )  = psi_bc(Dirichlet,lbc_type,z0b,tke(0   ,ith),z0b,u_taub)

   do i = 0,nlev
!     recover dissipation rate from k and psi
      eps(i,ith) = cm0**exp1 * tke(i,ith)**exp2 * psi(i)**exp3
