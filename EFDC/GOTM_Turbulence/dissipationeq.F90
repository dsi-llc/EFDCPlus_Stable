...
!
! !INTERFACE:
   subroutine dissipationeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
...
!  parameterisation for breaking surface waves suggested by Burchard (2001):
   ...
   if( sig_peps )then          ! With wave breaking
      sig_eff(nlev) = sig_e0
      do i = 1,nlev-1
         peps = (P(i,ith)+B(i,ith))/eps(i,ith)
   ...
   avh(0)    = num(0,   ith)/sig_e
   avh(nlev) = num(nlev,ith)/sig_e
!  compute RHS
   do i = 1,nlev-1

!     compute epsilon diffusivity
      avh(i) = num(i,ith)/sig_eff(i)

!     compute production terms in eps-equation
      if( B(i,ith).gt.0 )then
         ce3 = ce3plus
      else
         ce3 = ce3minus
      endif

      EpsOverTke  = eps(i,ith)/tkeo(i,ith)
      prod        = ce1*EpsOverTke*P(i,ith)
      buoyan      = ce3*EpsOverTke*B(i,ith)
      diss        = ce2*EpsOverTke*eps(i,ith)

      if( prod+buoyan.gt.0 )then
         Qsour(i)  = prod+buoyan
         Lsour(i) = -diss/eps(i,ith)
      else
         Qsour(i)  = prod
         Lsour(i) = -(diss-buoyan)/eps(i,ith)
      endif
...
!  do diffusion step
   call diff_face(nlev,dt,cnpar,h,psi_ubc,psi_lbc,                          &
                  DiffEpsup,DiffEpsdw,avh,Lsour,Qsour,eps(:,ith),ith)


!  fill top and bottom value with something nice
!  (only for output)
   eps(nlev,ith)  = epsilon_bc(Dirichlet,ubc_type,z0s,tke(nlev,ith),z0s,u_taus)
   eps(0   ,ith)  = epsilon_bc(Dirichlet,lbc_type,z0b,tke(0   ,ith),z0b,u_taub)
 
!  clip at eps_min
   do i = 0,nlev
      eps(i,ith) = max(eps(i,ith),eps_min)
   enddo
...
!        compute limit
         epslim = cde/sqrt(2.)/galp*tke(i,ith)*sqrt(NN_pos)

!        clip at limit
         eps(i,ith) = max(eps(i,ith),epslim)
...
!        compute dissipative scale
         Lgs(i,ith) = cde*sqrt(tke(i,ith)*tke(i,ith)*tke(i,ith))/eps(i,ith)
      enddo
   return
