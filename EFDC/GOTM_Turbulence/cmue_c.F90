...
!       clip an at minimum value
        an(i,ith) = max(an(i,ith),anLimitFact*anMin)

!       compute maximum as for shear instability
        asMaxNum  = d0*n0 + (d0*n1+d1*n0)*an(i,ith) + (d1*n1+d4*n0)*an(i,ith)*an(i,ith) + d4*n1*an(i,ith)*an(i,ith)*an(i,ith)
        asMaxDen  = d2*n0 + (d2*n1+d3*n0)*an(i,ith) + d3*n1*an(i,ith)*an(i,ith)
        asMax     = asMaxNum / asMaxDen

!       clip as at miximum value
        as(i,ith) = min(as(i,ith),asLimitFact*asMax)

        dCm  = d0  +  d1*an(i,ith) +  d2*as(i,ith) + d3*an(i,ith)*as(i,ith) + d4*an(i,ith)*an(i,ith) + d5*as(i,ith)*as(i,ith)
        nCm  = n0  +  n1*an(i,ith) +  n2*as(i,ith)
        nCmp = nt0 + nt1*an(i,ith) + nt2*as(i,ith)

        cmue1(i,ith) =  cm3_inv*nCm /dCm
        cmue2(i,ith) =  cm3_inv*nCmp/dCm
