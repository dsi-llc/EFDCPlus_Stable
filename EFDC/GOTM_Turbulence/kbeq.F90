...
!
! !INTERFACE:
   subroutine kbeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
...
!  compute diffusivity
   do i = 0,nlev
     avh(i) = nuh(i,ith)   ! *** Using loop for SGZ model in EFDC+
   enddo
...
!     compose source terms
      Qsour(i) =   prod_pos
      Lsour(i) =  (prod_neg - diss)/kb(i,ith)
...
!  do diffusion step
   call diff_face(nlev,dt,cnpar,h,k_ubc,k_lbc,                          &
                  DiffKbup,DiffKbdw,avh,Lsour,Qsour,kb(:,ith),ith)
