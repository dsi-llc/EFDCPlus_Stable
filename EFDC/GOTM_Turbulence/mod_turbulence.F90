...
! !PUBLIC MEMBER FUNCTIONS:
   public init_turbulence,post_init_turbulence,do_turbulence,init_turbulence_DSI
...
!  TKE, rate of dissipation, turbulent length-scale
   REALTYPE, public, dimension(:,:), allocatable, target :: tke,eps,Lgs
...
!  non-local fluxes of momentum
   !REALTYPE, public, dimension(:), allocatable   :: gamu,gamv  PMC

!  non-local fluxes
!  of buoyancy, temperature, salinity
   !REALTYPE, public, dimension(:), allocatable   :: gamb,gamh,gams    PMC
...
!  time scale ratio r
   !REALTYPE, public, dimension(:), allocatable   :: r     PMC

!  the gradient Richardson number
   !REALTYPE, public, dimension(:), allocatable   :: Rig   PMC
...
! **************************************************************************************************
! *** Define constants based on turbulence model selected

   REALTYPE, public :: z0s_min    !< Minimum Surface Roughness [m]
...
!
   ...
   namelist /turb_param/    cm0_fix,Prandtl0_fix,cw0,           &
                            compute_kappa,kappa,               &
...
80 FATAL 'I could not open "gotmturb.jnp"'
...
!BOC
   ...
   call twig%get(cw0, 'cw0', 'constant of the wave-breaking model', '-', &
   ...
   call twig%get(gen_alpha, 'gen_alpha', 'decay exponent alpha', '-', &
                   default = -2._rk)
   call twig%get(gen_l, 'gen_l', 'slope Lgs of the length scale in shear-free turbulence', '-', &
                   default = 0.2_rk)
...
!EOC
 Subroutine init_turbulence_DSI(nlev)
    ...
    use Variables_MPI, only: master_id, process_id
    use fson
...
!   the 'iw' namelist
    ...
    If( process_id == master_id )then
      json_data => fson_parse("gotm_turb.jnp")
      Write(*,'(A)') 'Turbulence closure: READING GOTM_TURB.JNP - TURBULENCE MODELS CONTROL FILE'
      
      call fson_get(json_data, "turbulence_method",                                            turb_method)
      call fson_get(json_data, "turbulent_kinetic_energy_eqn",                                 tke_method)
      call fson_get(json_data, "dissipative_length_scale",                                     len_scale_method)
      call fson_get(json_data, "stability_functions",                                          stab_method)
      
      call fson_get(json_data, "boundary_condition.upper_for_k_equation",                      k_ubc)
      call fson_get(json_data, "boundary_condition.lower_for_k_equation",                      k_lbc)
      call fson_get(json_data, "boundary_condition.upper_for_length_scale_equation",           psi_ubc)  
      call fson_get(json_data, "boundary_condition.lower_for_length_scale_equation",           psi_lbc)  
      call fson_get(json_data, "boundary_condition.upper_boundary_layer",                      ubc_type)  
      call fson_get(json_data, "boundary_condition.lower_boundary_layer",                      lbc_type)  
      
      call fson_get(json_data, "turbulence_param.log_law_stab_func",                           cm0_fix)
      call fson_get(json_data, "turbulence_param.Prandtl_number",                              Prandtl0_fix)
      call fson_get(json_data, "turbulence_param.wave_breaking_const",                         cw0)
      call fson_get(json_data, "turbulence_param.compute_von_karman_const",                    compute_kappa)
      call fson_get(json_data, "turbulence_param.von_karman_const",                            kappa)
      call fson_get(json_data, "turbulence_param.compute_c3",                                  compute_c3)
      call fson_get(json_data, "turbulence_param.steady_state_Richardson_number",              Ri_st)
      call fson_get(json_data, "turbulence_param.length_scale_limitation",                     length_lim)
      call fson_get(json_data, "turbulence_param.length_lim_coeff",                            galp)
      call fson_get(json_data, "turbulence_param.minimum_turbulent_kinetic_energy",            k_min)
      call fson_get(json_data, "turbulence_param.minimum_dissipation_rate",                    eps_min)
      call fson_get(json_data, "turbulence_param.minimum_buoyancy_variance",                   kb_min)
      call fson_get(json_data, "turbulence_param.minimum_buoyancy_variance_destruction_rate",  epsb_min)
      call fson_get(json_data, "turbulence_param.minimum_surface_roughness",                   z0s_min)
      
      call fson_get(json_data, "generic.compute_param",                                        compute_param)
      call fson_get(json_data, "generic.gen_m",                                                gen_m)
      call fson_get(json_data, "generic.gen_n",                                                gen_n)
      call fson_get(json_data, "generic.gen_p",                                                gen_p)
      call fson_get(json_data, "generic.cpsi1",                                                cpsi1)
      call fson_get(json_data, "generic.cpsi2",                                                cpsi2)
      call fson_get(json_data, "generic.cpsi3plus",                                            cpsi3plus)
      call fson_get(json_data, "generic.sig_kpsi",                                             sig_kpsi)
      call fson_get(json_data, "generic.sig_psi",                                              sig_psi)
      call fson_get(json_data, "generic.gen_d",                                                gen_d)
      call fson_get(json_data, "generic.gen_alpha",                                            gen_alpha)
      call fson_get(json_data, "generic.gen_l",                                                gen_l)
      
      call fson_get(json_data, "k_epsilon_param.dissipation_equation_ce1",                     ce1)
      call fson_get(json_data, "k_epsilon_param.dissipation_equation_ce2",                     ce2)
      call fson_get(json_data, "k_epsilon_param.stable_stratification_ce3minus",               ce3minus)
      call fson_get(json_data, "k_epsilon_param.unstable_stratification_ce3plus",              ce3plus)
      call fson_get(json_data, "k_epsilon_param.Schmidt_number_for_tke_diffusivity",           sig_k)
      call fson_get(json_data, "k_epsilon_param.Schmidt_number_for_dissipation_diffusivity",   sig_e)
      call fson_get(json_data, "k_epsilon_param.wave_breaking_parameterisation",               sig_peps)
      
      call fson_get(json_data, "my_param.e1",                                                  e1)
      call fson_get(json_data, "my_param.e2",                                                  e2)
      call fson_get(json_data, "my_param.e3",                                                  e3)
      call fson_get(json_data, "my_param.sq",                                                  sq)
      call fson_get(json_data, "my_param.sl",                                                  sl)
      call fson_get(json_data, "my_param.length",                                              my_length)
      call fson_get(json_data, "my_param.new_constr",                                          new_constr)
      
      
      call fson_get(json_data, "scnd_model.scnd_method",                                       scnd_method)
      call fson_get(json_data, "scnd_model.kb_method",                                         kb_method)
      call fson_get(json_data, "scnd_model.epsb_method",                                       epsb_method)
      call fson_get(json_data, "scnd_model.scnd_coeff",                                        scnd_coeff)
      call fson_get(json_data, "scnd_model.cc1",                                               cc1)
      call fson_get(json_data, "scnd_model.cc2",                                               cc2)
      call fson_get(json_data, "scnd_model.cc3",                                               cc3)
      call fson_get(json_data, "scnd_model.cc4",                                               cc4)
      call fson_get(json_data, "scnd_model.cc5",                                               cc5)
      call fson_get(json_data, "scnd_model.cc6",                                               cc6)
      call fson_get(json_data, "scnd_model.ct1",                                               ct1)
      call fson_get(json_data, "scnd_model.ct2",                                               ct2)
      call fson_get(json_data, "scnd_model.ct3",                                               ct3)
      call fson_get(json_data, "scnd_model.ct4",                                               ct4)
      call fson_get(json_data, "scnd_model.ct5",                                               ct5)
      call fson_get(json_data, "scnd_model.ctt",                                               ctt)
      
      ! *** Internal wave mixing - only for first-order turbulence closure
      if( turb_method == 2 )then
        call fson_get(json_data, "internal_wave_mixing.method",                                      iw_model)
        call fson_get(json_data, "internal_wave_mixing.Mellor_internal_wave_model_coeff",               alpha)
        call fson_get(json_data, "internal_wave_mixing.TKE_critical_value",                            klimiw)
        call fson_get(json_data, "internal_wave_mixing.shear_instability_critical_Richardson_number", rich_cr)
        call fson_get(json_data, "internal_wave_mixing.shear_instability_background_diffusivity",    numshear)
        call fson_get(json_data, "internal_wave_mixing.wave_breaking_background_viscosity",             numiw)
        call fson_get(json_data, "internal_wave_mixing.wave_breaking_background_diffusivity",           nuhiw)
      endif
      
    Endif
      
    call Broadcast_Scalar(turb_method,       master_id)
    call Broadcast_Scalar(tke_method,        master_id)
    call Broadcast_Scalar(len_scale_method,  master_id)
    call Broadcast_Scalar(stab_method,       master_id)
    
    call Broadcast_Scalar(k_ubc,             master_id)
    call Broadcast_Scalar(k_lbc,             master_id)
    call Broadcast_Scalar(psi_ubc,           master_id)
    call Broadcast_Scalar(psi_lbc,           master_id)
    call Broadcast_Scalar(ubc_type,          master_id)
    call Broadcast_Scalar(lbc_type,          master_id)
    
    call Broadcast_Scalar(cm0_fix,           master_id)
    call Broadcast_Scalar(Prandtl0_fix,      master_id)
    call Broadcast_Scalar(compute_kappa,     master_id)
    call Broadcast_Scalar(kappa,             master_id)
    call Broadcast_Scalar(compute_c3,        master_id)
    call Broadcast_Scalar(Ri_st,             master_id)
    call Broadcast_Scalar(length_lim,        master_id)
    call Broadcast_Scalar(galp,              master_id)
    call Broadcast_Scalar(k_min,             master_id)
    call Broadcast_Scalar(eps_min,           master_id)
    call Broadcast_Scalar(kb_min,            master_id)
    call Broadcast_Scalar(epsb_min,          master_id)
    call Broadcast_Scalar(z0s_min,           master_id)
    
    call Broadcast_Scalar(compute_param,     master_id)
    call Broadcast_Scalar(gen_m,             master_id)
    call Broadcast_Scalar(gen_n,             master_id)
    call Broadcast_Scalar(gen_p,             master_id)
    call Broadcast_Scalar(cpsi1,             master_id)
    call Broadcast_Scalar(cpsi2,             master_id)
    call Broadcast_Scalar(cpsi3plus,         master_id)
    call Broadcast_Scalar(sig_kpsi,          master_id)
    call Broadcast_Scalar(sig_psi,           master_id)
    call Broadcast_Scalar(gen_d,             master_id)
    call Broadcast_Scalar(gen_alpha,         master_id)
    call Broadcast_Scalar(gen_l,             master_id)
    
    call Broadcast_Scalar(ce1,               master_id)
    call Broadcast_Scalar(ce2,               master_id)
    call Broadcast_Scalar(ce3minus,          master_id)
    call Broadcast_Scalar(ce3plus,           master_id)
    call Broadcast_Scalar(sig_k,             master_id)
    call Broadcast_Scalar(sig_e,             master_id)
    call Broadcast_Scalar(sig_peps,          master_id)
    
    call Broadcast_Scalar(e1,                master_id)
    call Broadcast_Scalar(e2,                master_id)
    call Broadcast_Scalar(e3,                master_id)
    call Broadcast_Scalar(sq,                master_id)
    call Broadcast_Scalar(sl,                master_id)
    call Broadcast_Scalar(my_length,         master_id)
    call Broadcast_Scalar(new_constr,        master_id)
    
    call Broadcast_Scalar(scnd_method,       master_id)
    call Broadcast_Scalar(kb_method,         master_id)
    call Broadcast_Scalar(epsb_method,       master_id)
    call Broadcast_Scalar(scnd_coeff,        master_id)
    call Broadcast_Scalar(cc1,               master_id)
    call Broadcast_Scalar(cc2,               master_id)
    call Broadcast_Scalar(cc3,               master_id)
    call Broadcast_Scalar(cc4,               master_id)
    call Broadcast_Scalar(cc5,               master_id)
    call Broadcast_Scalar(cc6,               master_id)
    call Broadcast_Scalar(ct1,               master_id)
    call Broadcast_Scalar(ct2,               master_id)
    call Broadcast_Scalar(ct3,               master_id)
    call Broadcast_Scalar(ct4,               master_id)
    call Broadcast_Scalar(ct5,               master_id)
    call Broadcast_Scalar(ctt,               master_id)
    
    if( turb_method == 2 )then
      call Broadcast_Scalar(iw_model,          master_id)
      call Broadcast_Scalar(alpha,             master_id)
      call Broadcast_Scalar(klimiw,            master_id)
      call Broadcast_Scalar(rich_cr,           master_id)
      call Broadcast_Scalar(numshear,          master_id)
      call Broadcast_Scalar(numiw,             master_id)
      call Broadcast_Scalar(nuhiw,             master_id)
    endif
    
    if( present(nlev)) call post_init_turbulence(nlev)
    
End Subroutine init_turbulence_DSI
!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: Initialise the turbulence module
...
!  allocate memory
   ...
   !if (rc /= 0) stop 'init_turbulence: Error allocating (gamb)'     PMC
   !gamb = _ZERO_
   !
   ...
   allocate(cmue1(0:nlev, NTHREADS),stat = rc)
   if( rc /= 0) stop 'init_turbulence: Error allocating (cmue1)'
   cmue1 = _ZERO_

   allocate(cmue2(0:nlev, NTHREADS),stat = rc)
   ...
   allocate(an(0:nlev, NTHREADS),stat = rc)
   if( rc /= 0) stop 'init_turbulence: Error allocating (an)'
   an = _ZERO_

   allocate(as(0:nlev, NTHREADS),stat = rc)
   if( rc /= 0) stop 'init_turbulence: Error allocating (as)'
   as = _ZERO_

   allocate(at(0:nlev, NTHREADS),stat = rc)
   ...
   allocate(xRf(0:nlev, NTHREADS),stat = rc)
   if( rc /= 0) stop 'init_turbulence: Error allocating (xRf)'
   xRf = _ZERO_

   !allocate(uu(0:nlev),stat = rc)
   !if (rc /= 0) stop 'init_turbulence: Error allocating (uu)'   PMC
   !uu = _ZERO_
   !
...
!  report on parameters and properties of the model
   if( process_id == master_id ) call report_model
...
! \begin{equation}
!   \label{power_law}
!   k = K (z+z_0)^\alpha \comma l = Lgs(z+z_0)
!   \comma
! \end{equation}
...
! \begin{equation}
!   \label{GE_alphaL_1}
!   (\alpha Lgs)^2 = \frac{2}{3}   (c_\mu^0)^2 R \, \sigma_k^\psi
!   \comma
! \end{equation}
...
!   \label{GE_n}
!   \begin{array}{rcl}
!     n & = & - \dfrac{1}{4 (2+d) (\kappa^2 R - Lgs^2)}
!     \Bigg(
!       4 d \kappa^2 R \,  m - (1+4m) (2+d) \alpha Lgs^2
!       \\
!     &+&
!       \left.  \sqrt{ 8 m (1+2m) (2+d)^2 (\kappa^2 R - Lgs^2) \alpha^2 Lgs^2
!     + \Big( - 4 d \kappa^2 R \ m + (2+d) (1+4m) \alpha Lgs^2   \Big)^2
!        }
!     \right)
!     \point
!   \end{array}
...
! do some checks
   ...
   if( gen_l.lt.0. )then
      STDERR 'Slope Lgs of the length scale in shear-free'
      STDERR 'turbulence has to be positive.'
      STDERR 'Measured values are: 0.15 < Lgs < 0.25'
      STDERR 'Please change gotm_turb.jnp accordingly'
      STDERR 'Program aborts now in turbulence.F90'
      stop
   endif
...
! check for negative radicand and compute n
   if( rad.gt.0 )then
      ...
      STDERR 'Please choose other value for gen_m in gotm_turb.jnp'
      STDERR 'Program aborts now in turbulence.F90'
      stop
   endif
...
! check for negative Schmidt-number sig_psi
   if( sig_psi.lt.0. )then
      ...
      STDERR 'Possible reason: You set gen_d < -2 in gotm_turb.jnp?'
      STDERR 'Please change gotm_turb.jnp accordingly'
      STDERR 'Program aborts now in turbulence.F90'
      stop
   endif
...
! Report on the properties of each model
   select case(len_scale_method)
      case(Parabolic)
         ...
         LEVEL3 'length-scale slope (no shear), Lgs      = ', gen_l
         LEVEL2 '--------------------------------------------------------'
         LEVEL2 ' '
      case(Triangular)
         LEVEL2 ' '
      ...
      case(Robert_Ouellet)
         ...
         LEVEL3 'length-scale slope (no shear),     Lgs = not computed'
         LEVEL2 '--------------------------------------------------------'
         LEVEL2 ' '
      case(Blackadar)
         LEVEL2 ' '
...
!
! !INTERFACE:
   subroutine do_turbulence(ith,nlev,dt,depth,u_taus,u_taub,z0s,z0b,h,      &
                            NN,SS,xP)
!
...
! !USES:
   ...
   interface
      subroutine production(ith,nlev,NN,SS,xP)
        integer,  intent(in)                :: nlev
...
!        with seagrass turbulence
         call production(ith,nlev,NN,SS,xP)
      else
!        without
         call production(ith,nlev,NN,SS)
      endif

      call alpha_mnb(nlev,NN,SS,ith)
      call stabilityfunctions(nlev,ith)
      call do_tke(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      call do_lengthscale(nlev,dt,depth,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      call kolpran(nlev,ith)

      call internal_wave(nlev,NN,SS,ith)
...
!        without
      ...
      case (quasi_Eq)
         ...
         call alpha_mnb(nlev,NN,SS,ith)
         call cmue_d(nlev,ith)
         call do_tke(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
         call do_kb(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
         call do_lengthscale(nlev,dt,depth,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
         call do_epsb(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
         call alpha_mnb(nlev,NN,SS,ith)
         call kolpran(nlev,ith)
      ...
      STDERR 'Choose scnd_method = 1,2 in gotm_turb.jnp.'
      STDERR 'Program execution stopped ...'
      stop 'turbulence.F90'
      ...
      STDERR 'Choose scnd_method = 1,2,3 in gotm_turb.jnp.'
      STDERR 'Program execution stopped ...'
      stop 'turbulence.F90'
...
!
! !INTERFACE:
   subroutine do_tke(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
!
! !DESCRIPTION:
   ...
   select case (tke_method)
      case(tke_local_eq)
         ! use algebraic length scale equation
          call tkealgebraic(nlev,u_taus,u_taub,NN,SS,ith)
      case(tke_keps)
         ! use differential equation for tke (k-epsilon style)
         call tkeeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case(tke_MY)
         ! use differential equation for q^2/2 (Mellor-Yamada style)
         call q2over2eq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case default
   end select
...
!
! !INTERFACE:
   subroutine do_kb(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
!
! !DESCRIPTION:
...
!BOC
   select case (kb_method)
      case(kb_algebraic)
         call kbalgebraic(nlev,ith)
      case(kb_dynamic)
         call kbeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case default

      STDERR '... not a valid method to compute kb'
      STDERR 'Choose  kb_method = 1,2 in gotm_turb.jnp'
      STDERR 'Program execution stopped ...'
      stop 'turbulence.F90'
...
! !INTERFACE:
   ...
   subroutine do_lengthscale(nlev,dt,depth,u_taus,u_taub, z0s,z0b,h,NN,SS,ith)
!
! !DESCRIPTION:
...
!BOC
   select case(len_scale_method)
      case(diss_eq)
         call dissipationeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case(generic_eq)
         call genericeq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case(length_eq)
         call lengthscaleeq(nlev,dt,depth,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
      case(Bougeault_Andre)
         call potentialml(nlev,z0b,z0s,h,depth,NN,ith)
      case default
         call algebraiclength(len_scale_method,nlev,z0b,z0s,depth,h,NN,ith)
   end select
...
!
! !INTERFACE:
   subroutine do_epsb(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
!
! !DESCRIPTION:
...
!BOC
   select case (epsb_method)
      case(epsb_algebraic)
         ...
         call epsbalgebraic(nlev,ith)

      case(epsb_dynamic)

         STDERR '... sorry, epsb_method = 2 not yet implemented.'
         STDERR 'Choose  epsb_method = 1 in gotm_turb.jnp'
         STDERR 'Program execution stopped ...'
         stop 'turbulence.F90'
      ...
      case default
         ...
         STDERR 'Choose  epsb_method = 1,2 in gotm_turb.jnp'
         STDERR 'Program execution stopped ...'
         stop 'turbulence.F90'
...
!  update the turbulent diffusivities
   do i = 0,nlev
      x        =  sqrt(tke(i,ith))*Lgs(i,ith)
!     momentum
      num(i,ith)   =  cmue1(i,ith)*x
!     heat
      nuh(i,ith)   =  cmue2(i,ith)*x
!     salinity
      nus(i,ith)   =  cmue2(i,ith)*x
   enddo
...
!
! !INTERFACE:
   subroutine stabilityfunctions(nlev,ith)
!
! !DESCRIPTION:
...
! formally set the values at the boundaries
   cmue1(0,ith)      = cmue1(1,ith)
   cmue1(nlev,ith)   = cmue1(nlev-1,ith)
   cmue2(0,ith)      = cmue2(1,ith)
   cmue2(nlev,ith)   = cmue2(nlev-1,ith)
...
!   - \dfrac{c_\mu}{\sigma_k} K^\frac{3}{2} Lgs \alpha (z+z_0)^{\frac{3}{2} \alpha}
!   \comma
! \end{equation}
...
! \begin{equation}
!   \label{K}
!   K = \left( -\dfrac{\sigma_k}{c_\mu \alpha Lgs} F_k \right)^\frac{2}{3} \dfrac{1}{z_0^\alpha}
!  \comma
! \end{equation}
...
!   - \sqrt{2} S_q  K^\frac{3}{2} \alpha Lgs (z+ z_0)^{\frac{3}{2} \alpha}
!   \comma
! \end{equation}
...
! \begin{equation}
!   \label{MY_K}
!   K = \left( - \dfrac{F_q}{\sqrt{2} S_q \alpha Lgs} \right)^\frac{2}{3}
!        \dfrac{1}{z_0^\alpha}
!    \comma
! \end{equation}
...
!   \epsilon= (c_\mu^0)^3 K^\frac{3}{2} Lgs^{-1} (z+z_0)^{\frac{3}{2}\alpha - 1}
!   \point
! \end{equation}
!
...
!   \psi= (c_\mu^0)^p K^m  Lgs^n (z+z_0)^{m \alpha + n}
!   \point
! \end{equation}
!
! The Neumann (flux) boundary condition is
...
!   F_l = - S_l q l \partder{q^2 l}{z}  = 
!   - 2 \sqrt{2} S_l  (\alpha+1) K^\frac{3}{2}  Lgs^2 (z+ z_0)^{\frac{3}{2} \alpha + 1}
!   \comma
! \end{equation}
...
!BOC
   ...
   if( allocated(Lgs)) deallocate(Lgs)
...
!BOC
   ...
   LEVEL2 'tke,eps,Lgs',tke,eps,Lgs
   LEVEL2 'tkeo',tkeo
   ...
   LEVEL2 'turb_param namelist', cm0_fix,Prandtl0_fix,cw0,           &
                            compute_kappa,kappa,               &
                            compute_c3,ri_st,length_lim,galp,  &
                            const_num,const_nuh,k_min,eps_min, &
                            kb_min,epsb_min
