...
!
! !INTERFACE:
   subroutine q2over2eq(nlev,dt,u_taus,u_taub,z0s,z0b,h,NN,SS,ith)
...
! !USES:
   ...
   use turbulence,   only: tke,tkeo,k_min,eps,Lgs
...
!     compute diffusivity
      avh(i) = sq*sqrt( 2.*tke(i,ith) )*Lgs(i,ith)
...
!     compute production terms in q^2/2-equation
      ...
      if( prod+buoyan.gt.0 )then
         Qsour(i)  = prod+buoyan
         Lsour(i) = -diss/tke(i,ith)
      else
         Qsour(i)  = prod
         Lsour(i) = -(diss-buoyan)/tke(i,ith)
      endif
...
!  do diffusion step
   call diff_face(nlev,dt,cnpar,h,k_ubc,k_lbc,                          &
                    DiffKup,DiffKdw,avh,Lsour,Qsour,tke(:,ith),ith)

!  fill top and bottom value with something nice
!  (only for output)
   tke(nlev,ith)  = q2over2_bc(Dirichlet,ubc_type,z0s,z0s,u_taus)
   tke(0   ,ith)  = q2over2_bc(Dirichlet,lbc_type,z0b,z0b,u_taub)
