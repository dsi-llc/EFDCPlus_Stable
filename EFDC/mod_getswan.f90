...
MODULE GETSWANMOD
  ...
  LOOPJ: DO J=1,JC
    DO I=1,IC
      ...
      IF( WV(L).HEISIG >= WHMI .AND. WVPRD > 0. )THEN    ! DELME - FOR SWAN WHERE DO WE INITIALIZE "WVPRD"                           
  ...
  DO N=1,NLOC
    ...
    !WV(L).DISSIPA(KC) = EDIS/RHO                    ![M3/S3] 
    IF( WV(L).HEISIG >= WHMI .AND. WVPRD > 0 )THEN       ! DELME - WVPRD?                   
      WVDX= COS(WDIR*PI/180)
      WVDY= SIN(WDIR*PI/180)
      WVCX =  CVN(L)*WVDX - CVE(L)*WVDY         ! TO LOCAL CURVI-LINEAR SYSTEM
      WVCY = -CUN(L)*WVDX + CUE(L)*WVDY
