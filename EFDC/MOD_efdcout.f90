...
! ----------------------------------------------------------------------
  ...
  IF( ISSPH(8) >= 1 )THEN
    ...
    WRITE(EE_UNIT) INT(NDYE,4),INT(NSED,4),INT(NSND,4),INT(NTOX,4)
...
SUBROUTINE WCOUT
  ...
  IF( ISTRAN(6) >= 1 .OR. ISTRAN(7) >= 1 )THEN
    ...
    IF( ISTRAN(6) >= 1 ) WRITE(EE_UNIT) (((REAL(SED_Global(L,K,NS),4), K=KSZ_Global(L),KC), L=2,LA_Global), NS=1,NSED)
    IF( ISTRAN(7) >= 1 )THEN
