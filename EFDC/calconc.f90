...
SUBROUTINE CALCONC(ISTL_,IS2TL_)
  ...
  DO IW=1,NACTIVEWC
    ...
    CALL CALTRAN(ISTL_, IS2TL_, IACTIVEWC1(IW), IACTIVEWC2(IW), WCV(IW).VAL0, WCV(IW).VAL1, IW, IT, WCV(IW).WCLIMIT, ISKIP(IW))
  ...
  DO IW=1,NACTIVEWC
    IF( ISKIP(IW) == 0 )THEN
      !$  IT = OMP_GET_THREAD_NUM() + 1
      CALL CALTRAN_AD(ISTL_, IS2TL_, IACTIVEWC1(IW), IACTIVEWC2(IW), WCV(IW).VAL0, WCV(IW).VAL1, IW, IT)
    ENDIF
  ENDDO
  !$OMP END DO
  ...
  IF( ISICE == 4 .AND. (IS2TL > 0 .OR. (IS2TL == 0 .AND. NCTBC /= NTSTBC)) )THEN
    IF( LFRAZIL )THEN
      ...
      CALL CALTRANICE(ISTL_,FRAZILICE,FRAZILICE1,1)
      NICE = NICE+1
