...
SUBROUTINE VARINIT
  ...
  IF( ISPROPWASH > 0 .AND. ISTRAN(6) > 0 .AND. fraction_fast > 0 .AND. fast_multiplier > 0. )THEN
    ...
    NSCM2 = NSCM*2         ! *** Maximum array sizes.  May not need all of these but will be determined later
  ENDIF

  ! *** Time series pointers for different constituents
  Call AllocateDSI( MSVDYE,  NDYM,   0)
  Call AllocateDSI( MSVSED,  NSCM2,  0)
  ...
  NSTM2  = MAX(3, NSCM2 + NSNM2 + NTXM)                      ! *** Maximum number of SedTox variables with fast settling classes
  NSTVM  = MAX(7, 3 + NDYM + NSCM  + NSNM  + NTXM + NWQV)    ! *** Maximum number of constituents
  NSTVM2 = MAX(7, 3 + NDYM + NSCM2 + NSNM2 + NTXM + NWQV)    ! *** Maximum number of constituents with fast settling classes
